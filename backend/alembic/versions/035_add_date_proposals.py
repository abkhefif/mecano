...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("buyer_id", sa.UUID(), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True),
        sa.Column("mechanic_id", sa.UUID(), sa.ForeignKey("mechanic_profiles.id", ondelete="CASCADE"), nullable=False, index=True),
        ...
        sa.Column("parent_id", sa.UUID(), sa.ForeignKey("date_proposals.id", ondelete="SET NULL"), nullable=True),
